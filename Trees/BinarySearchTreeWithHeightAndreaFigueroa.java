 ...
 * Due: 10/18/2023
 * Source code, filled out the height(TreeNode<E> root) 
 * method to have it return the height of the binary search tree*/
import java.util.*;

public class BinarySearchTreeWithHeightAndreaFigueroa {
...
class BSTWithHeight<E> extends BST<E> {
  ...
  private int height(TreeNode<E> root) {
    // WRITE YOUR CODE HERE
	  //if there is no root node, the height is 0
	  if(root == null) { //base case
		  return 0;
	  } else {
		  /*uses recursion to go through all the leftmost nodes in the tree,
		   * starts by going through the left branch first and then moves onto
		   * the right branch */
		  int leftHeight = height(root.left);
		  int rightHeight = height(root.right);
		  
		  //returns the biggest height +1 to include the starting root node
